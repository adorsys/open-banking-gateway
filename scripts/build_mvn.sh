...
if [[ "$TRAVIS_EVENT_NAME" == "cron"* || "$TRAVIS_BRANCH" == *"heavy-tests-enabled"* || "$TRAVIS_PULL_REQUEST_BRANCH" == *"heavy-tests-enabled"* ]]; then
    echo "Heavy tests enabled"
    export ENABLE_HEAVY_TESTS="true";
fi

mvn verify --no-transfer-progress -Djgiven.report.text=false
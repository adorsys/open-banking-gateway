...
class OpbaApiWithHbciConsentUiSmokeE2ETest extends SpringScenarioTest<SmokeSandboxServers, WebDriverBasedAccountInformation<? extends WebDriverBasedAccountInformation<?>>, AccountInformationResult> {
    ...
    @Test
    void testAccountsListWithConsentNoScaButUserHasMultiSca(FirefoxDriver firefoxDriver) {
        given()
                .rest_assured_points_to_opba_server_with_fintech_signer_on_banking_api(config.getOpbaServerUri());

        when()
                .fintech_calls_list_accounts_for_max_musterman(HBCI_SANDBOX_BANK_SCA_ID)
                .and()
                ...
                .and()
                .user_provided_to_consent_ui_initial_parameters_to_list_accounts_with_hbci_consent(firefoxDriver, "anton.brueckner")
                .and()
                .user_in_consent_ui_reviews_account_consent_and_accepts(firefoxDriver)
                .and()
                .user_in_consent_ui_provides_pin(firefoxDriver, "12345")
                .and()
        ...
        then()
                ...
                .open_banking_can_read_max_musterman_hbci_account_data_using_consent_bound_to_service_session(HBCI_SANDBOX_BANK_SCA_ID);
    }
}

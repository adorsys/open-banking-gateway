...
public class AuthorizeService {
    ...
    public Optional<UserEntity> findUser(LoginRequest loginRequest) {
        ...
        if (userProfile != null) {
            // password matched
            // create new session
            userEntity.setXsrfID(UUID.randomUUID().toString());
            userEntity.setCookie("cookie gnerated by finTechServer " + UUID.randomUUID());

            // Entity will now be found be xrefid too
            xsrfIDtoEntityMap.put(userEntity.getXsrfID(), userEntity);

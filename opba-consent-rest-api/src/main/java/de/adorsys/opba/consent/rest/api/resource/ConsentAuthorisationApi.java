...
public interface ConsentAuthorisationApi {
    ...
    @ApiOperation(value = "Entry point for processing a consent request redirected by the TppBankingApi over the FinTechApi to this ConsentAuthorisationApi.",
            notes = "This is the <b>entry point</b> for processing a consent redirected by the TppBankingApi to this ConsentAuthorisationApi."
                    ...
                    + "</ul>"
                    + "<ul><b>Initial request processing ConsentAuthorisationApi</b>"
                    ...
                    + "<li>All subsequent request to the ConsentAuthorisationApi must specify the consentAuthState.</li>"
                    + "</ul>"
                    ...
                    + "<li>The ConsentAuthSessionCookie is httpOnly</li>"
                    + "</ul>"
                    + "<ul><b>Redirecting PSU to the ASPSP</b>"
                    ...
                    + "</ul>"
                    + "<ul><b>Back-Redirecting PSU to the ConsentAuthorisationApi</b>"
                    ...
                    + "</ul>"
                    + "<ul><b>Back-Redirecting PSU to the FinTechApi</b>"
    ...
    @ApiOperation(value = "Provides the ConsentAuthorisationApi with the opportunity to redirect the PSU to the ASPSP.",
            notes = "Provides the ConsentAuthorisationApi with the opportunity to redirect the PSU to the ASPSP."
                    ...
                    + "<li>ConsentAuthSessionCookie.</li>"
                    + "</ul>"
                    + "<ul><b>Returns:</b>"
                    + "<li>302 Redirect</li>"
                    + "<li>Redirect Location Header.</li>"
                    + "<li>New ConsentAuthSessionCookie.</li>"
                    + "</ul>",
            authorizations = @Authorization(value = API_KEY))
    ResponseEntity<Void> grantRedirect(
            @RequestHeader(name = COOKIE, required = false) String consentAuthSessionCookie,
            @RequestBody AuthorizeRequest request);


    /**
     * Provides the ConsentAuthorisationApi with the opportunity to close this ConsentSession and
     * redirect the PSU back to the TPP <b>OR</b> close the application window.
     * <p>
     * In any case, the ConsentSession of the PSU will be terminated and cookies will be deleted.
     *
    ...
    @ApiOperation(value = "Closes this session and redirects the PSU back to the FinTechApi or close the application window.",
            notes = "Closes this session and redirects the PSU back to the FinTechApi or close the application window. "
                    ...
                    + "<ul><b>Returns:</b>"
                    + "<li>302 Redirect</li>"
                    + "<li>Redirect Location Header to tpp.</li>"
                    + "<li>Null ConsentAuthSessionCookie (deletion).</li>"
                    + "</ul>",
            authorizations = @Authorization(value = API_KEY))
    ResponseEntity<Void> denyRedirect(
            @RequestHeader(name = COOKIE, required = false) String consentAuthSessionCookie,
    ...
    @ApiOperation(value = "Redirecting back from ASPSP to ConsentAuthorisationApi after a successful consent authorization.",
            notes = "Redirecting back from ASPSP to ConsentAuthorisationApi after a successful consent authorization. "
                    ...
                    + "<ul><b>Returns:</b>"
                    + "<li>302 Redirect</li>"
                    + "<li>Redirect Location Header to FinTechApi.</li>"
                    + "<li>Null ConsentAuthSessionCookie (deletion).</li>"
                    + "</ul>",
            authorizations = @Authorization(value = API_KEY))
    ResponseEntity<AuthorizeResponse> fromAspspOk(
    ...
    @ApiOperation(value = "Redirecting back from ASPSP to TPP after a failed consent authorization.",
            notes = "Redirecting back from ASPSP to TPP after a failed consent authorization. "
                    + "In any case, the session of the user will be closed and cookies will be deleted with the response to this request."
                    + "<ul><b>Request contains:</b>"
                    ...
                    + "</ul>",
            authorizations = @Authorization(value = API_KEY))
    ResponseEntity<AuthorizeResponse> fromAspspNok(
            @PathVariable(CONSENT_SESSION_STATE) String consentSessionState,
            @RequestHeader(name = COOKIE, required = false) String consentAuthSessionCookie);
    ...
    @ApiOperation(value = "Generic challenge response end point for updating consent session with PSU authentication data while requesting remaining challenges for the ongoing authorization process.",
            notes = "Update consent session with PSU auth data whereby requesting remaining challenges for the ongoing authorization process."
                    + "<ul><b>Request contains:</b>"
                    + "<li>consentSessionState is retrieved from the response of the peceeding request.</li>"
                    + "<li>The PsuAuthRequest constaining necessary consent info and auth data.</li>"
                    + "</ul>"
                    + "<ul><b>Returns:</b>"
                    + "<li>The AuthorizeResponse constaining necessary consent info.</li>"
                    + "</ul>",
            authorizations = @Authorization(value = API_KEY))
